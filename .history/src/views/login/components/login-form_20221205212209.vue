...
import { userAccoutLogin } from "@/api/user";
...
const login = async () => {
  ...
  if (valid) {
    ...
    userAccoutLogin(account, password)
      .then((res: any) => {
        console.log(res);
        const { id, accout, avatar, mobile, nickname, token } = res.result;
        userStore.setUser({ id, accout, avatar, mobile, nickname, token });
      })
      .cathc((err: ErrorEvent) => {});
  }
};
</script>
