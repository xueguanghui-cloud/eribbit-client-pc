...
const brand = reactive({
  id: "brand",
  name: "品牌",
  children: [{ id: "brand-children", name: "品牌推荐", goods:[], children:[], picture: "", open: false }],
  brands: []
})
// 得到9个分类，且每个一级分类只有两个
    ...
    <ul class="menu">
      <li v-for="menu in menuList" :key="menu.id" @mouseenter="categoryId = menu.id" :class="{active: categoryId === menu.id}">
        ...
        <template v-if="menu.children.length">
          <RouterLink v-for="sub in menu.children" :key="sub.id" :to="`/category/sub/${sub.id}`">{{ sub.name }}</RouterLink>
        </template>
        <!--  骨架 -->
        <template v-else>
          <XghSkeleton width="60px" height="18px" style="margin-right:5px" bg="rgba(255,255,255,0.2)"  />
